    ...
    <div class="py-12" x-data="{ 
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div x-show="width < 768" class="space-y-4">
                @forelse($conductores as $conductor)
                    <div class="bg-gray-800 rounded-lg p-4 border border-gray-700 shadow-sm flex flex-col gap-3">
                        <div class="flex items-start gap-3">
                            <div class="h-12 w-12 flex-shrink-0 rounded-full overflow-hidden border border-gray-500">
                                @if($conductor->fotografia)
                                    <img src="{{ asset('storage/' . $conductor->fotografia) }}"
                                        class="h-full w-full object-cover">
                                @else
                            ...
                            <div class="flex-1 min-w-0">
                                <h3 class="text-sm font-bold text-white truncate">{{ $conductor->nombre }}</h3>
                                <p class="text-xs text-gray-400 truncate">{{ $conductor->cargo }}</p>
                                <p class="text-xs text-gray-500 truncate">{{ $conductor->departamento }}</p>
                            </div>
                            @php $vencida = $conductor->fecha_licencia->isPast(); @endphp
                            <div class="flex-shrink-0">
                                <span
                                    class="px-2 py-0.5 inline-flex text-[10px] font-bold rounded-lg {{ $vencida ? 'bg-red-900/30 text-red-400' : 'bg-emerald-900/30 text-emerald-400' }}">
                        ...
                        <div
                            class="text-xs text-gray-400 bg-gray-900/50 p-2 rounded border border-gray-700/50 flex justify-between">
                            <span>Vencimiento Licencia:</span>
                            <span
                                class="{{ $vencida ? 'text-red-400 font-bold' : 'text-gray-300' }}">{{ $conductor->fecha_licencia->format('d/m/Y') }}</span>
                        </div>
                        ...
                        <div class="flex justify-end gap-3 border-t border-gray-700 pt-3">
                            ...
                            <button
                                @click="deleteAction = '{{ route('conductores.destroy', $conductor) }}'; openDeleteModal = true"
                                class="p-2 bg-gray-700 rounded-lg text-red-400 hover:bg-gray-600 transition-colors">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16">
                                    </path>
                ...
                @empty
                    <div class="bg-gray-800 rounded-lg p-8 text-center text-gray-500 border border-gray-700">
